...
class RaftNode:
    ...
    def __init__(self, env: simpy.Environment, name: str, cluster: List):
    ...
    def run(self) -> Generator:
        while True:
            if random.random() <= NODE_PROBABILITY_FAILURE:
                ...
                # else:
                yield self.env.all_of([sleeping_timeout])
                self.is_shutdown = False
            if self.state == FOLLOWER:
                write_to_file(f'{self.name} стал фоловером', self.env)
                ...
                self.term += 1
                write_to_file(f'{self.name} стал кандидатом и запрашивает голоса',  self.env)
                ...
                result = yield self.env.any_of([self.heartbit_event, self.entry_event, self.ack_log_event])  # ждем либо когда будем посылать хартбит, либо когда придёт запрос, либо чужой хартбит от вохможного лидера

    def heartbit_callback(self, event) -> None:
    ...
    def ack_log_callback(self, event) -> None:
        ...
        if not self.is_shutdown and not self.leader.is_shutdown and self.term <= self.leader.term: # что с индексом? если терм принимающего выше?
            if self.state == LEADER:  # если получили терм поновее(или такой же?) зануляем запросы на лидера и передаем их новому
                self.leader.entries = self.entries
                self.entries = 0
            self.state = FOLLOWER
            if self.leader.entries > 0 and self.index + 1 != self.leader.index:  # фоловер не синхронизирован
                ...
                self.index = self.leader.index - 1  # синхронизировали(доработка)
    ...
    def commit_log_callback(self, event) -> None:
        if not self.is_shutdown and not self.leader.is_shutdown and self.term <= self.leader.term: # and self.index + 1 == self.leader.index ?
...
def run_simulation():
    ...
    cluster = [RaftNode(env, f"Узел {i}", []) for i in range(5)]
