...
import datetime
from logs.logs_setup import setup_logging, get_logger
...
class RaftNode:
	...
	def __init__(self, env: simpy.Environment, name: str, cluster: List, metrics: Metrics, num_nodes: int, settings: dict):
		...
		self.routing_event: simpy.Event = self.env.event()
		self.routing_event.callbacks.append(self.routing_callback)
	...
	def run(self) -> Generator:
		while True:
			if self.ack_log_event.triggered:
				self.logger.info(f'ack log event{self.ack_log_event.triggered}')
			if self.commit_log_event.triggered:
				self.logger.info(f'commit log event{self.commit_log_event.triggered}')
			if self.requested_vote_event.triggered:
				self.logger.info(f'req vote event{self.requested_vote_event.triggered}')
			if self.routing_event.triggered:
				self.logger.info(f'rout event{self.routing_event.triggered}')
			if not self.is_shutdown:
				if self.state == FOLLOWER:
					...
					result = yield self.env.any_of([self.timeout_proc, self.ack_log_event, self.commit_log_event, self.requested_vote_event, self.routing_event]) #возвращает словарь с тригер событиями?
					...
					result = yield self.env.any_of([self.nodes_voted_timeout_event, self.ack_log_event, self.requested_vote_event, self.routing_event])  #  ждём таймаут выборов, или когда проснется лидер, или когда(возможно) к нам постучаться проголосовать 
				elif self.state == LEADER:
					#write_to_file(f'{self.name} лидер', self.env, RaftNode.timestamp)
					self.logger.info(f'{self.env.now} Лидер')
					result = yield self.env.any_of([self.heartbit_proc, self.entry_proc, self.ack_log_event, self.routing_event])  # ждем либо когда будем посылать хартбит, либо когда придёт запрос, либо чужой хартбит от вохможного лидера
					for event in result:
	...
	def shutdown_loop(self):
		while True:
			...
			self.network_manager.disable_node(self.node_number)
			...
			self.network_manager.enable_node(self.node_number)
	...
	def heartbit_callback(self) -> None:
		if not self.is_shutdown:
			...
			for i in self.cluster:
				if i != self:
					next = self.network_manager.get_next_hop(self.node_number, i.node_number)
					if next is None:
						self.logger.warning(f'{self.env.now} Лидер не может послать heartbeat из-за сбоя в сети')
					else:
						#event = i.ack_log_event
						#self.network_manager.send(self.node_number, next, callback=lambda next=next, i=i: self.cluster[next].trigger_routing_event((self, i, None, lambda src=self, event=event: event.succeed(src))))
						self.network_manager.send(self.node_number, next, callback=lambda next=next, i=i: self.cluster[next].trigger_routing_event((self, i, HEARTBEAT, (i.ack_log_event.succeed, self))))
				...
				if self.ack_count >= len(self.cluster) // 2:  # подтвердило большинство
					...
					for i in self.cluster:
						if i != self:
							...
							next = self.network_manager.get_next_hop(self.node_number, i.node_number)
							if next is None:
								self.logger.warning(f'Лидер не может отправить коммиты')
							else:
								#event = i.commit_log_event
								#self.network_manager.send(self.node_number, next, callback=lambda next=next: self.cluster[next].trigger_routing_event(value=(self, i, None, lambda src=self: event.succeed(src))))
								self.network_manager.send(self.node_number, next, callback=lambda next=next, i=i: self.cluster[next].trigger_routing_event(value=(self, i, COMMIT, (i.commit_log_event.succeed, self))))

					RaftNode.processed_entries += self.sending_num  # считаем запрос закомиченым лидером
					self.entries -= self.sending_num
				else:
					#write_to_file('Запись не подтвердилась большинством', self.env, RaftNode.timestamp)
					self.logger.info(f'{self.env.now} Запись не подтвердилась большинством')
	...
	def entry_callback(self, leader) -> None:
		...
		self.logger.info(f'{self.env.now} Лидер не принял запрос, необработанных записей: {self.entries}')
	...
	def trigger_routing_event(self, value):
		event = self.routing_event

		self.set_new_routing_event()

		if not self.is_shutdown:
			self.logger.info(
				f"{self.env.now} routing_event {id(event)} for {self.node_number}"
			)

		event.succeed(value)


	def routing_callback(self, event) -> None:
		if not self.is_shutdown:
			self.set_new_routing_event()
			src = event.value[0]
			dst = event.value[1]
			do = event.value[2]
			delegate, attr = event.value[3]
			if dst == self:
				if do in (HEARTBEAT, COMMIT, ACK, VOTE_ANS, VOTE_REQ, SYN_ANS):
					if delegate is not None:
						try:
							delegate(attr)
						except Exception as e:
							self.logger.critical(f'{self.env.now} Исключение в delegate {do} {delegate.__name__}: {e}')
					else:
						self.logger.warning(f'{self.env.now} delegate пустой {do}')
				elif do == SYN_REQ:#отправка обратно
					(src, dst) = (dst, src)
					next = self.network_manager.get_next_hop(src.node_number, dst.node_number)
					if next is None:
						self.logger.info(f'Пакеты syn не смогли отправится от узла {src} к узлу {dst.node_number}')    
						return
					self.network_manager.send(src.node_number, next, callback=lambda src=src, dst=dst, next=next, delegate=delegate, attr=attr: self.cluster[next].trigger_routing_event((src, dst, SYN_ANS, (delegate, attr))))
			else: # к следующему узлу
				next = self.network_manager.get_next_hop(self.node_number, dst.node_number)
				if next is None:
					self.logger.warning(f'{self.env.now} Потерян пакет при отправке')
				else:
					self.network_manager.send(self.node_number, next, callback=lambda src=src, next=next, dst=dst, do=do, delegate=delegate, attr=attr: self.cluster[next].trigger_routing_event((src, dst, do, (delegate, attr))))
		else:
			self.logger.warning(f'Потерян пакет на узле')

	def ack_log_callback(self, event):
		...
		if not self.is_shutdown and not self.leader.is_shutdown and self.term <= self.leader.term:
			...
			if self.leader.sending_num > 0 and self.index != self.leader.index:  # фоловер не синхронизирован
				...
				next = self.network_manager.get_next_hop(self.node_number, self.leader.node_number)
				if next is None:
					self.logger.warning(f'{self.env.now} Пакеты для запроса syn не отправились к узлу {self.leader.node_number}')
				else:
					self.logger.info(f'{self.env.now}СОБИРАЮСЬ ПОСЛАТЬ SYN')
					self.network_manager.send(self.node_number, next, callback=lambda next=next, dst=self.leader, syn=syn_index: self.cluster[next].trigger_routing_event((self, dst, SYN_REQ, (syn, None))))
			...
			if self.leader.sending_num > 0:  # если это не простой хартбит
				...
				def ack(*args):
					...
					self.logger.info(f'{self.env.now} Подтвердил запрос {self.leader.name}, терм лидера {self.leader.term}, индекс лидера {self.leader.index}, терм узла {self.term} индекс узла {self.index}')
				
				next = self.network_manager.get_next_hop(self.node_number, self.leader.node_number)
				if next is None:
					self.logger.warning(f'{self.env.now} Не смог подтвердить запрос {self.leader.name} из-за сбоя в сети')
				else:
					self.network_manager.send(self.node_number, next, callback=lambda next=next, dst=self.leader, ack=ack:(self.cluster[next].trigger_routing_event((self, dst, ACK, (ack, None)))))
		...
		if not self.is_shutdown: 
			...
			if not self.voted and self.state != LEADER:
				...
				if (candidate.term == self.term and candidate.index >= self.index) or candidate.term > self.term:
					...
					# вызываем через сеть
					next = self.network_manager.get_next_hop(self.node_number, candidate.node_number)
					if next is None:
						self.logger.warning(f'{self.env.now} Не смог проголосовать за {self.leader.name} из-за сбоя в сети')
					else:
						self.network_manager.send(self.node_number, next, callback=lambda next=next, vote=vote, dst=candidate: self.cluster[next].trigger_routing_event((self, dst, VOTE_ANS, (vote, None))))
	...
	def request_votes(self) -> None:
		...
		for node in self.cluster:  # посылаем запросы
			if node != self:
				next = self.network_manager.get_next_hop(self.node_number, node.node_number)
				if next is None:
					self.logger.warning(f'{self.env.now} не смог запросить голоса из-за сбоя в сети')
					...
					#self.network_manager.send(self.node_number, next, callback=lambda next=next, src=self, dst=node: self.cluster[next].trigger_routing_event((src, dst, None, lambda dst=node, src=src: dst.requested_vote_event.succeed(src))))
					self.network_manager.send(self.node_number, next, callback=lambda next=next, src=self, dst=node: self.cluster[next].trigger_routing_event((src, dst, VOTE_REQ, (dst.requested_vote_event.succeed, self))))
	...
	def set_new_routing_event(self):
		self.routing_event = self.env.event()
		self.routing_event.callbacks.append(self.routing_callback)

# при выборе топологии кол во узлов не меняется
# отправка не работает, enable не учитывается
